...
1.  **全局 Top-K 选边（label-free）**：对 `G_{retrieval}` 的所有边按分数排序，直接保留 `edge_top_k` 条高分边。
2.  **诱导子图 + Triple 去重**：对保留下来的边诱导子图，再按三元组键 `(head_entity_id, relation_id, tail_entity_id)` 去重；`edge_scores` 用 max 聚合、`edge_labels` 用 max 聚合（实现见 `src/data/components/g_agent_builder.py`）。
3.  **Hop 裁剪 + 监督**：在去重后的子图上对 `start_node_locals` 做无向 BFS，保留 `max_hops` 内的节点/边；随后为每个 `(start, answer)` 对生成最短路 DAG 掩码（`pair_*` CSR + `edge_labels`）。
4.  **GT 字段（可选，缺失静默）**：若 LMDB 提供 `gt_path_edge_indices`（或 `gt_paths_triples`）且其边恰好落在裁剪后的子图内，则写入 `gt_path_*`；否则 `gt_path_exists=false` 且 `gt_path_*` 为空张量。缺失 GT 不报错、不影响物化流程。
...
*   **数据模块**：`configs/data/gflownet.yaml` 读取 `{split}_g_agent.pt`，自动 padding 出批次级别的实体/关系 ID 与图结构。
