...
def relation_action(rel: str, rules: RelationCleaningRules, *, enabled: bool) -> str:
    ...
    if rel in rules.type_exact or any(
        rel.startswith(prefix) for prefix in rules.type_prefixes
    ):
        return RELATION_ACTION_TYPE
    if any(pattern.search(rel) for pattern in rules.type_regexes):
        return RELATION_ACTION_TYPE
    if rel in rules.drop_exact or any(
        rel.startswith(prefix) for prefix in rules.drop_prefixes
    ):
