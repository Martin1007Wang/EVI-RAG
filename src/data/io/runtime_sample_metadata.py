...
_RUNTIME_SAMPLE_METADATA_VERSION = 1
...
def runtime_sample_metadata_path(embeddings_dir: Path, split: str) -> Path:
    return Path(embeddings_dir) / f"{str(split)}.metadata.pt"


def save_runtime_sample_metadata(
    path: Path,
    *,
    split: str,
    ...
    payload = _build_runtime_sample_metadata_payload(
        split=split,
...
def load_runtime_sample_metadata(path: Path) -> RuntimeSampleMetadata:
    payload = torch.load(path, map_location="cpu")
    if not isinstance(payload, dict):
        raise ValueError(
            f"Runtime sample metadata at {path} must decode to a dict, got {type(payload)!r}."
        )
    version = int(payload.get("version", -1))
    if version != _RUNTIME_SAMPLE_METADATA_VERSION:
        raise ValueError(
            "Unsupported runtime sample metadata version: "
            f"expected {_RUNTIME_SAMPLE_METADATA_VERSION}, got {version}."
        )
    ...
    question_tokens = _coerce_int_tensor(
        payload.get("question_tokens"), name="question_tokens"
    )
    _validate_lengths(
        sample_ids=sample_ids,
        questions=questions,
...
def _build_runtime_sample_metadata_payload(
    *,
    split: str,
    sample_ids: Sequence[str],
    questions: Sequence[str],
    ...
    _validate_unique_sample_ids(sample_ids)
    return {
        "version": _RUNTIME_SAMPLE_METADATA_VERSION,
        "split": str(split),
        "sample_ids": sample_ids,
        "questions": questions,
        "num_nodes": num_nodes_tensor,
...
def _coerce_int_tensor(value: object, *, name: str) -> torch.Tensor:
    ...
    if tensor.dim() != 1:
        raise ValueError(
            f"Runtime sample metadata field {name!r} must be 1D, got {tuple(tensor.shape)}."
        )
    if tensor.dtype not in (
        ...
        raise TypeError(
            f"Runtime sample metadata field {name!r} must be integral, got {tensor.dtype}."
        )
    return tensor
...
def _validate_lengths(
    *,
    ...
    mismatched = {
        name: actual for name, actual in actual_lengths.items() if actual != expected
    }
    if mismatched:
        raise ValueError(
            "Runtime sample metadata length mismatch: "
            f"expected {expected} entries, got {mismatched}."
        )
...
def _validate_unique_sample_ids(sample_ids: Sequence[str]) -> None:
    ...
    if duplicates:
        raise ValueError(
            "Runtime sample metadata contains duplicate sample_ids, examples: "
            f"{duplicates}."
        )
