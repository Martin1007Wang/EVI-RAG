...
class DataResource:
    def __init__(self, entity_metadata_path: Path, embeddings_dir: Path) -> None:
        self.entity_metadata_path = Path(entity_metadata_path)
        self.embeddings_dir = Path(embeddings_dir)
        self.embedding_store = EmbeddingStore(self.embeddings_dir)
        self.entity_embedding_map, self.cvt_mask = _load_entity_metadata(
            self.entity_metadata_path
        )
...
def _load_entity_metadata(path: Path) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    if not torch.is_tensor(entity_embedding_map) or entity_embedding_map.dtype != torch.long:
