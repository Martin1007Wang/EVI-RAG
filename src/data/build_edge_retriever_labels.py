...
from src.data.g_retrieval_dataset import create_g_retrieval_dataset
...
def _resolve_output_dir(cfg: DictConfig) -> Path:
    ...
    return artifact_dir / "edge_retriever_labels"


def _build_split(cfg: DictConfig, *, split: str, output_dir: Path, overwrite: bool) -> Path:
    ...
    dataset = create_g_retrieval_dataset(cfg=dataset_cfg, split_name=split, resources=None)
...
@hydra.main(version_base="1.3", config_path="../../configs", config_name="build_edge_retriever_labels.yaml")
def main(cfg: DictConfig) -> None:
    ...
    for split in splits:
        path = _build_split(cfg, split=split, output_dir=output_dir, overwrite=overwrite)
