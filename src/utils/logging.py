...
class RankedLogger(std_logging.LoggerAdapter):
...
def log_hyperparameters(object_dict: Dict[str, Any]) -> None:
    ...
    hparams["model/params/trainable"] = sum(_safe_numel(p) for p in params if p.requires_grad)
    hparams["model/params/non_trainable"] = sum(_safe_numel(p) for p in params if not p.requires_grad)
...
__all__ = ["RankedLogger", "log_hyperparameters", "infer_batch_size", "log_metric"]
