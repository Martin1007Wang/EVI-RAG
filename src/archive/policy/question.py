...
def build_question_context_tokens(
    *,
    ...
    question_ctx = observation.question_context
    ...
    if int(question_ctx.size(0)) != int(observation.num_graphs):
        raise ValueError(
            "question_ctx batch mismatch with num_graphs: "
            f"question_ctx={int(question_ctx.size(0))}, num_graphs={int(observation.num_graphs)}."
        )
...
def build_question_padding_mask(
    *,
    ...
    raw_mask = observation.question_valid_mask
    if raw_mask is None:
        raise ValueError(
...
def compute_agent_potentials(
    *,
    ...
    batch_size = int(observation.num_graphs)
    agent_graph_ids = torch.arange(
        batch_size, device=question_tokens.device, dtype=torch.long
    ).repeat_interleave(num_agents)
    return compute_agent_potentials_from_graph_ids(
        ...
        question_lexical_proj=question_lexical_proj,
        question_cross_attention=question_cross_attention,
        question_cross_attention_norm=question_cross_attention_norm,
...
def compute_agent_potentials_from_graph_ids(
    *,
    ...
    batch_size = int(observation.num_graphs)
    total_agents = int(agent_graph_ids.numel())
    if agent_graph_ids.dim() != 1:
        raise ValueError(
            f"agent_graph_ids must be 1D, got shape={tuple(agent_graph_ids.shape)}."
        )
    if bool((agent_graph_ids < 0).any().item()) or bool(
        (agent_graph_ids >= batch_size).any().item()
    ):
        raise ValueError("agent_graph_ids contains out-of-range graph indices.")
    if int(agent_history.size(0)) != total_agents:
        raise ValueError(
            "agent_history row count must match agent_graph_ids length in potential computation."
        )
    ...
    query = (
        path_to_policy(agent_history).unsqueeze(1).to(dtype=torch.float32).contiguous()
    )
    context_fp32 = agent_question_context.to(dtype=torch.float32).contiguous()
    if lexical_question_tokens is None:
        ...
        if int(lexical_question_tokens.size(0)) != batch_size:
            raise ValueError(
                "lexical_question_tokens batch mismatch with num_graphs: "
                f"lexical={int(lexical_question_tokens.size(0))}, num_graphs={batch_size}."
            )
...
__all__ = [
    ...
    "compute_agent_potentials_from_graph_ids",
