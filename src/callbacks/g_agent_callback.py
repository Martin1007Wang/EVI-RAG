...
from types import SimpleNamespace # <--- 引入这个
...
class GAgentGenerationCallback(Callback):
    ...
    def on_predict_batch_end(
        ...
        # Tasteful Adaptation: Dictionary -> Object
        # 我们的 RetrieverModule.predict_step 返回的是 Dict
        # 但 GAgentBuilder 可能期待对象属性访问 (outputs.scores)
        model_output = outputs
        if isinstance(outputs, dict):
            # 使用 SimpleNamespace 优雅地将字典转换为对象
