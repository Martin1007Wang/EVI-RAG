...
def compute_llm_metrics(
    *,
    ...
    for record in _iter_jsonl(input_path):
        ...
        if sample is None:
            raise ValueError(
                "Missing or invalid gold labels for predicted sample_id=" f"{sample_id!r}. Provide a valid labels sidecar via llm.input_labels_path."
            )
    ...
    if unknown_pred_ids:
        ...
        raise ValueError(
            f"Predictions contain sample_id values not found in input JSONL {input_path}: " f"{preview} (showing up to {_MAX_ERROR_IDS_TO_SHOW})."
        )
    ...
    if full_acc.samples > _ZERO:
        metrics["llm/input/trajectory_count_mean"] = float(trajectory_count_sum) / float(full_acc.samples)
    ...
    if sub_acc.hal_stats is not None:
        metrics.update(_format_hal_stats(sub_acc.hal_stats, prefix="llm/subgraphrag/sub/stats"))
    return metrics
...
def _evaluate_sample(sample: _EvalSample) -> _EvalResult:
    ...
    no_ans = _subgraphrag_no_answer_in_graph(sample.pred_text, sample.pred_lines)
    return _EvalResult(
        hit_at_1=hit_at_1,
        hit=hit,
        f1=f1,
...
def _subgraphrag_no_answer_in_graph(prediction: str, pred_lines: Sequence[str]) -> bool:
    if pred_lines:
        return False
