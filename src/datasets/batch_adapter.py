...
def require_edge_index(value: Any, *, device: torch.device) -> torch.Tensor:
    ...
    if tensor.dim() != 2 or tensor.size(0) != 2:
        raise ValueError(
            f"edge_index must have shape [2, E], got {tuple(tensor.shape)}"
        )
    return tensor
...
def require_str_list(value: Any, *, expected_size: int, field: str) -> list[str]:
    ...
    if len(value) != expected_size:
        raise ValueError(
            f"{field} length mismatch: expected {expected_size}, got {len(value)}"
        )
    return list(value)


def coerce_optional_str_list(
    value: Any, *, expected_size: int, field: str, default: str = ""
) -> list[str]:
    if value is None:
...
def extract_graph_tensors(
    ...
    node_ptr = require_1d_long(
        getattr(batch, "node_ptr", None), device=device, name="node_ptr"
    )
    edge_index = require_edge_index(getattr(batch, "edge_index", None), device=device)
    edge_rel_global = require_1d_long(
        getattr(batch, "edge_attr", None), device=device, name="edge_attr"
    )
    ...
    edge_batch = require_1d_long(
        getattr(batch, "edge_batch", None), device=device, name="edge_batch"
    )
    node_batch = require_1d_long(
        getattr(batch, "batch", None), device=device, name="batch"
    )
...
def extract_alignment_tensors(
    ...
    q_local_indices = require_1d_long(
        getattr(batch, "q_local_indices", None), device=device, name="q_local_indices"
    )
    a_local_indices = require_1d_long(
        getattr(batch, "a_local_indices", None), device=device, name="a_local_indices"
    )
    ...
    answer_ptr = require_1d_long(
        getattr(batch, "answer_ptr", None), device=device, name="answer_ptr"
    )
...
def extract_embeddings(
    ...
    node_embeddings = require_2d_float(
        getattr(batch, "node_embeddings", None), device=device, name="node_embeddings"
    )
    edge_embeddings = require_2d_float(
        getattr(batch, "edge_embeddings", None), device=device, name="edge_embeddings"
    )
    question_emb = require_2d_float(
        getattr(batch, "question_emb", None), device=device, name="question_emb"
    )
...
def extract_optional_heuristic_log_v(
    batch: Any, *, device: torch.device
) -> torch.Tensor | None:
    ...
    *,
    ...
    if raw_question_ctx is None:
        raise ValueError(
            ...
            "Data preprocessing must provide per-token question embeddings."
        )
    question_ctx = require_3d_float(
        raw_question_ctx, device=device, name="question_ctx"
    )
    ...
    if raw_mask is None:
        question_ctx_mask = torch.ones(
            question_ctx.shape[:2], dtype=torch.bool, device=device
        )
    else:
        question_ctx_mask = require_bool_2d(
            raw_mask, device=device, name="question_ctx_mask"
        )
...
def extract_metadata(
    ...
    node_global_ids = require_1d_long(
        getattr(batch, "node_global_ids", None), device=device, name="node_global_ids"
    )
    dummy_mask = require_bool_1d(
        getattr(batch, "dummy_mask", None), device=device, name="dummy_mask"
    )
    if int(dummy_mask.numel()) != num_graphs:
        raise ValueError("dummy_mask length mismatch with num_graphs.")
    sample_ids = require_str_list(
        getattr(batch, "sample_id", None), expected_size=num_graphs, field="sample_id"
    )
...
class _SuperSourceLayout:
    node_counts: torch.Tensor
    q_counts: torch.Tensor
    a_counts: torch.Tensor
    num_graphs: int
    graph_ids: torch.Tensor
    num_super_nodes_per_graph: int
    new_node_ptr: torch.Tensor
    forward_super_abs: torch.Tensor
    backward_super_abs: torch.Tensor


def _build_super_source_layout(
    *,
    ...
    if bool((q_counts <= 0).any().item()):
        raise ValueError(
            "super_source_enabled requires non-empty q_local_indices per graph."
        )
    ...
    forward_super_abs = new_node_ptr[1:] - 2
    backward_super_abs = new_node_ptr[1:] - 1
    return _SuperSourceLayout(
        node_counts=node_counts,
        q_counts=q_counts,
        a_counts=a_counts,
        num_graphs=num_graphs,
        graph_ids=graph_ids,
        num_super_nodes_per_graph=num_super_nodes_per_graph,
        new_node_ptr=new_node_ptr,
        forward_super_abs=forward_super_abs,
        backward_super_abs=backward_super_abs,
    )


def _augment_node_tensors(
    *,
    layout: _SuperSourceLayout,
    ...
    new_num_nodes = old_num_nodes + layout.num_graphs * layout.num_super_nodes_per_graph
    old_to_new = (
        torch.arange(old_num_nodes, device=node_batch.device, dtype=torch.long)
        + node_batch * layout.num_super_nodes_per_graph
    )
    new_node_embeddings = node_embeddings.new_zeros(
        (new_num_nodes, int(node_embeddings.size(1)))
    )
    ...
    new_node_embeddings.index_copy_(0, layout.forward_super_abs, question_emb)
    new_node_embeddings.index_copy_(0, layout.backward_super_abs, question_emb)

    new_node_batch = torch.empty(
        (new_num_nodes,), device=node_batch.device, dtype=torch.long
    )
    new_node_batch.index_copy_(0, old_to_new, node_batch)
    new_node_batch.index_copy_(0, layout.forward_super_abs, layout.graph_ids)
    new_node_batch.index_copy_(0, layout.backward_super_abs, layout.graph_ids)
...
def _shift_edge_index(
    ...
    *,
    ...
    return torch.stack([edge_index[0] + src_shift, edge_index[1] + dst_shift], dim=0)


def _build_super_edge_index(
    *,
    layout: _SuperSourceLayout,
    q_local_indices: torch.Tensor,
    a_local_indices: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    q_offsets = layout.new_node_ptr[:-1].repeat_interleave(layout.q_counts)
    q_abs = q_local_indices + q_offsets
    forward_super_heads = layout.forward_super_abs.repeat_interleave(layout.q_counts)
    forward_super_edge_index = torch.stack([forward_super_heads, q_abs], dim=0)

    a_offsets = layout.new_node_ptr[:-1].repeat_interleave(layout.a_counts)
    a_abs = a_local_indices + a_offsets
    backward_super_heads = layout.backward_super_abs.repeat_interleave(layout.a_counts)
    backward_super_edge_index = torch.stack([a_abs, backward_super_heads], dim=0)
    return (
        forward_super_edge_index,
        backward_super_edge_index,
        forward_super_heads,
        backward_super_heads,
    )


def _combine_edge_index(
    *,
    shifted_edge_index: torch.Tensor,
    forward_super_edge_index: torch.Tensor,
    backward_super_edge_index: torch.Tensor,
    forward_super_heads: torch.Tensor,
    backward_super_heads: torch.Tensor,
) -> torch.Tensor:
...
def _augment_edge_tensors(
    *,
    ...
    base_super_rel_id = (
        int(edge_rel_global.max().item()) + 1 if edge_rel_global.numel() > 0 else 0
    )
    new_edge_rel_global = edge_rel_global
    if int(forward_super_heads.numel()) > 0:
        forward_super_rel = edge_rel_global.new_full(
            (forward_super_heads.numel(),), fill_value=base_super_rel_id
        )
    ...
    if int(backward_super_heads.numel()) > 0:
        ...
        new_edge_rel_global = torch.cat(
            [new_edge_rel_global, backward_super_rel], dim=0
        )
    ...
    if int(forward_super_heads.numel()) > 0:
        forward_super_edge_embeddings = edge_embeddings.new_zeros(
            (forward_super_heads.numel(), rel_dim)
        )
        new_edge_embeddings = torch.cat(
            [new_edge_embeddings, forward_super_edge_embeddings], dim=0
        )
        new_edge_batch = torch.cat(
            [
                new_edge_batch,
                layout.graph_ids.repeat_interleave(layout.q_counts),
            ],
            dim=0,
        )
    if int(backward_super_heads.numel()) > 0:
        backward_super_edge_embeddings = edge_embeddings.new_zeros(
            (backward_super_heads.numel(), rel_dim)
        )
        new_edge_embeddings = torch.cat(
            [new_edge_embeddings, backward_super_edge_embeddings], dim=0
        )
        new_edge_batch = torch.cat(
            [
                new_edge_batch,
                layout.graph_ids.repeat_interleave(layout.a_counts),
            ],
            dim=0,
        )
    return new_edge_rel_global, new_edge_embeddings, new_edge_batch
...
class DualFlowBatchAdapter:
    ...
    def build_relation_tokens(
        ...
        if edge_rel_global.numel() == 0:
            ...
            return edge_embeddings.new_zeros((0, rel_dim)), edge_rel_global.new_zeros(
                (0,)
            )

        _, edge_relations = torch.unique(
            edge_rel_global, sorted=True, return_inverse=True
        )
        num_rel = int(edge_relations.max().item()) + 1
        edge_ids = torch.arange(
            edge_relations.numel(), device=edge_relations.device, dtype=torch.long
        )
        first_occ = torch.full(
            (num_rel,),
            edge_relations.numel(),
    ...
    def maybe_attach_super_source(
        ...
        *,
        ...
        layout = _build_super_source_layout(
            node_ptr=node_ptr,
            q_ptr=q_ptr,
            a_ptr=a_ptr,
        )
        ...
        shifted_edge_index = _shift_edge_index(
            edge_index,
            node_batch=node_batch,
            num_super_nodes_per_graph=layout.num_super_nodes_per_graph,
        )
        (
            forward_super_edge_index,
            backward_super_edge_index,
            forward_super_heads,
            backward_super_heads,
        ) = _build_super_edge_index(
            layout=layout,
            q_local_indices=q_local_indices,
            a_local_indices=a_local_indices,
        )
        new_edge_index = _combine_edge_index(
            shifted_edge_index=shifted_edge_index,
            forward_super_edge_index=forward_super_edge_index,
            backward_super_edge_index=backward_super_edge_index,
            forward_super_heads=forward_super_heads,
            backward_super_heads=backward_super_heads,
        )
        new_edge_rel_global, new_edge_embeddings, new_edge_batch = (
            _augment_edge_tensors(
        ...
        new_relation_tokens, new_edge_relations = self.build_relation_tokens(
            new_edge_rel_global, new_edge_embeddings
        )
        return (
            layout.new_node_ptr,
            new_edge_index,
    ...
    def prepare_batch(
        ...
        node_ptr, edge_index, edge_rel_global, edge_batch, node_batch = (
            extract_graph_tensors(batch, device=device)
        )
        (
            q_local_indices,
            a_local_indices,
            ...
            answer_ptr,
            answer_entity_ids,
        ) = extract_alignment_tensors(batch, device=device)
        node_embeddings, edge_embeddings, question_emb = extract_embeddings(
            batch, device=device, num_graphs=num_graphs
        )
        heuristic_log_v = extract_optional_heuristic_log_v(batch, device=device)
        question_ctx, question_ctx_mask = extract_question_context(
            batch,
            device=device,
            num_graphs=num_graphs,
        )
        if heuristic_log_v is not None and int(heuristic_log_v.numel()) != int(
        ...
        relation_tokens, edge_relations = self.build_relation_tokens(
            edge_rel_global, edge_embeddings
        )
