...
class GRetrievalDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> GRetrievalData:
        if torch.is_tensor(idx):
            if idx.dim() != 0:
                raise TypeError(
                    "GRetrievalDataset only supports integer indexing; batching belongs in DataLoader."
                )
...
def create_g_retrieval_dataset(
    ...
    *,
    ...
    filter_missing_start = _resolve_split_bool(
        cfg, split_name, "filter_missing_start", True
    )
    filter_missing_answer = _resolve_split_bool(
        cfg, split_name, "filter_missing_answer", True
    )
...
def _resolve_missing_filter_paths(
    ...
    *,
    ...
    if not isinstance(paths, dict) or not paths.get("processed"):
        raise ValueError(
            "dataset_cfg.paths.processed is required when filter_missing_* is enabled."
        )
