# src/models/rollout/sampler.py
"""
[系统实体] 并行 Rollout 采样门面
职责：
1. 统一暴露 sample_forward / evaluate_forced_paths / beam_search_forward 三类接口。
2. 将具体执行委托给独立引擎，避免单文件过度膨胀。
"""

from __future__ import annotations

import torch
from torch import nn

from src.models.configs.search import RolloutConfig
from src.models.environment import FlowDirection, GraphEnvContext

from src.models.policy import DualFlowPolicy

from .engines import BeamDecoderEngine, OfflineForcedEvalEngine, OnlineRolloutEngine
from .types import RolloutResult

EncodedPolicyContext = tuple[torch.Tensor, torch.Tensor, torch.Tensor]


class ActionSampler(nn.Module):
    """将 ragged 边 logits 打包为逐 agent 的分类分布并采样动作。"""

    _MIN_TEMPERATURE = 1e-4
    _NEAR_ZERO_TEMPERATURE = 1e-3

    def __init__(self) -> None:
        super().__init__()

    @staticmethod
    def _sanitize_logits(
        *,
        logits: torch.Tensor,
    ) -> torch.Tensor:
        if logits.dim() != 2:
            raise ValueError(
                f"logits must be 2D [num_rows, num_actions], got {tuple(logits.shape)}"
            )
        if int(logits.size(1)) == 0:
            raise ValueError("logits must contain at least one action column.")
        has_finite = torch.isfinite(logits).any(dim=1)
        has_nan = torch.isnan(logits).any(dim=1)
        has_pos_inf = torch.isposinf(logits).any(dim=1)
        invalid_rows = has_nan | has_pos_inf | (~has_finite)
        if not bool(invalid_rows.any().item()):
            return logits
        safe_logits = logits.clone()
        neg_inf = torch.tensor(float("-inf"), device=logits.device, dtype=logits.dtype)
        stop_index = int(logits.size(1)) - 1
        safe_logits[invalid_rows] = neg_inf
        safe_logits[invalid_rows, stop_index] = 0.0
        return safe_logits

    def _build_final_logits(
        self,
        *,
        edge_logits: torch.Tensor,
        out_degrees: torch.Tensor,
        stop_logits: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, int]:
        total_agents = out_degrees.size(0)
        device = edge_logits.device
        neg_inf = torch.tensor(float("-inf"), device=device, dtype=edge_logits.dtype)
        max_deg = int(out_degrees.max().item()) if total_agents > 0 else 0
        padded_logits = torch.full(
            (total_agents, max_deg), neg_inf, device=device, dtype=edge_logits.dtype
        )
        if max_deg > 0:
            mask = torch.arange(max_deg, device=device).unsqueeze(
                0
            ) < out_degrees.unsqueeze(1)
            padded_logits[mask] = edge_logits
        final_logits = torch.cat([padded_logits, stop_logits], dim=-1)
        final_logits = self._sanitize_logits(logits=final_logits)
        log_partition = torch.logsumexp(final_logits, dim=-1)
        stop_index = int(final_logits.size(1)) - 1
        stop_log_prob = final_logits[:, stop_index] - log_partition
        return final_logits, log_partition, stop_log_prob, max_deg

    def _select_action_idx(
        self,
        *,
        final_logits: torch.Tensor,
        is_training: bool,
        deterministic: bool,
        sampling_mode: str,
        sampling_temperature: float,
        eval_sampling_temperature: float,
        eval_sample_without_replacement: bool,
        agent_graph_ids: torch.Tensor | None,
        source_nodes: torch.Tensor | None,
        active_mask: torch.Tensor | None,
        num_nodes_total: int,
        temperature: float | None,
    ) -> torch.Tensor:
        if temperature is not None:
            actual_temp = temperature
        else:
            actual_temp = (
                eval_sampling_temperature if deterministic else sampling_temperature
            )
        actual_temp = float(max(actual_temp, self._MIN_TEMPERATURE))
        if is_training and abs(actual_temp - 1.0) > 1e-6:
            raise ValueError(
                "On-policy SubTB requires sampling_temperature == 1.0 during training "
                f"(got {actual_temp})."
            )
        mode = str(sampling_mode).strip().lower()
        if actual_temp <= self._NEAR_ZERO_TEMPERATURE or mode == "greedy":
            return final_logits.argmax(dim=-1)
        scaled_logits = final_logits / actual_temp
        dist = torch.distributions.Categorical(
            logits=scaled_logits, validate_args=False
        )
        if not is_training and eval_sample_without_replacement:
            return self._sample_eval_actions_without_replacement(
                final_logits=scaled_logits,
                active_mask=active_mask,
                agent_graph_ids=agent_graph_ids,
                source_nodes=source_nodes,
                num_nodes_total=num_nodes_total,
                enable_grouped_without_replacement=True,
            )
        return dist.sample()

    @staticmethod
    def _empty_action_output(
        *,
        action_idx: torch.Tensor,
        log_partition: torch.Tensor,
        stop_log_prob: torch.Tensor,
        dtype: torch.dtype,
    ) -> dict[str, torch.Tensor]:
        return {
            "is_stop": torch.ones_like(action_idx, dtype=torch.bool),
            "chosen_edge_ids": torch.full_like(action_idx, -1),
            "chosen_target_nodes": torch.zeros_like(action_idx),
            "log_prob": torch.zeros_like(action_idx, dtype=dtype),
            "log_partition": log_partition,
            "stop_log_prob": stop_log_prob,
        }

    @staticmethod
    def _build_action_output(
        *,
        action_idx: torch.Tensor,
        max_deg: int,
        out_degrees: torch.Tensor,
        edge_ids: torch.Tensor,
        target_nodes: torch.Tensor,
        final_logits: torch.Tensor,
        log_partition: torch.Tensor,
        stop_log_prob: torch.Tensor,
    ) -> dict[str, torch.Tensor]:
        is_stop = action_idx == max_deg
        base_offsets = out_degrees.cumsum(0) - out_degrees
        flat_chosen_idx = base_offsets + action_idx
        safe_flat_idx = flat_chosen_idx.clamp(max=max(0, edge_ids.numel() - 1))
        chosen_edge_ids = edge_ids[safe_flat_idx]
        chosen_target_nodes = target_nodes[safe_flat_idx]
        chosen_edge_ids = torch.where(
            is_stop, torch.full_like(chosen_edge_ids, -1), chosen_edge_ids
        )
        chosen_target_nodes = torch.where(
            is_stop, torch.zeros_like(chosen_target_nodes), chosen_target_nodes
        )
        true_dist = torch.distributions.Categorical(
            logits=final_logits, validate_args=False
        )
        return {
            "is_stop": is_stop,
            "chosen_edge_ids": chosen_edge_ids,
            "chosen_target_nodes": chosen_target_nodes,
            "log_prob": true_dist.log_prob(action_idx),
            "log_partition": log_partition,
            "stop_log_prob": stop_log_prob,
        }

    @staticmethod
    def _unpack_policy_output(
        policy_output: dict[str, torch.Tensor],
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        edge_logits = policy_output["edge_logits"]
        out_degrees = policy_output["out_degrees"].view(-1)
        stop_logits = policy_output["stop_logits"].view(-1, 1)
        edge_ids = policy_output["edge_ids"]
        target_nodes = policy_output["target_nodes"]
        return edge_logits, out_degrees, stop_logits, edge_ids, target_nodes

    def forward(
        self,
        policy_output: dict[str, torch.Tensor],
        *,
        is_training: bool,
        deterministic: bool,
        sampling_mode: str,
        sampling_temperature: float,
        eval_sampling_temperature: float,
        eval_sample_without_replacement: bool,
        agent_graph_ids: torch.Tensor | None = None,
        source_nodes: torch.Tensor | None = None,
        active_mask: torch.Tensor | None = None,
        num_nodes_total: int = 0,
        temperature: float | None = None,
    ) -> dict[str, torch.Tensor]:
        edge_logits, out_degrees, stop_logits, edge_ids, target_nodes = (
            self._unpack_policy_output(policy_output)
        )

        final_logits, log_partition, stop_log_prob, max_deg = self._build_final_logits(
            edge_logits=edge_logits,
            out_degrees=out_degrees,
            stop_logits=stop_logits,
        )
        action_idx = self._select_action_idx(
            final_logits=final_logits,
            is_training=is_training,
            deterministic=deterministic,
            sampling_mode=sampling_mode,
            sampling_temperature=sampling_temperature,
            eval_sampling_temperature=eval_sampling_temperature,
            eval_sample_without_replacement=eval_sample_without_replacement,
            agent_graph_ids=agent_graph_ids,
            source_nodes=source_nodes,
            active_mask=active_mask,
            num_nodes_total=num_nodes_total,
            temperature=temperature,
        )

        if edge_ids.numel() == 0:
            return self._empty_action_output(
                action_idx=action_idx,
                log_partition=log_partition,
                stop_log_prob=stop_log_prob,
                dtype=edge_logits.dtype,
            )

        return self._build_action_output(
            action_idx=action_idx,
            max_deg=max_deg,
            out_degrees=out_degrees,
            edge_ids=edge_ids,
            target_nodes=target_nodes,
            final_logits=final_logits,
            log_partition=log_partition,
            stop_log_prob=stop_log_prob,
        )

    def _sample_eval_actions_without_replacement(
        self,
        *,
        final_logits: torch.Tensor,
        active_mask: torch.Tensor | None,
        agent_graph_ids: torch.Tensor | None,
        source_nodes: torch.Tensor | None,
        num_nodes_total: int,
        enable_grouped_without_replacement: bool,
    ) -> torch.Tensor:
        num_rows, num_actions = final_logits.shape
        stop_index = num_actions - 1
        action_idx = torch.full(
            (num_rows,),
            fill_value=stop_index,
            device=final_logits.device,
            dtype=torch.long,
        )
        if num_rows == 0:
            return action_idx
        active_mask, active_rows = self._resolve_active_rows(
            active_mask=active_mask,
            num_rows=num_rows,
            device=final_logits.device,
        )
        if int(active_rows.numel()) == 0:
            return action_idx
        if (
            not enable_grouped_without_replacement
            or agent_graph_ids is None
            or source_nodes is None
        ):
            sampled = self._sample_independent_actions(
                logits=final_logits,
                active_rows=active_rows,
            )
            action_idx[active_rows] = sampled
            return action_idx
        grouped_actions = self._sample_grouped_actions(
            logits=final_logits,
            active_rows=active_rows,
            agent_graph_ids=agent_graph_ids,
            source_nodes=source_nodes,
            num_nodes_total=num_nodes_total,
            stop_index=stop_index,
        )
        action_idx[active_rows] = grouped_actions
        return action_idx

    @staticmethod
    def _resolve_active_rows(
        *,
        active_mask: torch.Tensor | None,
        num_rows: int,
        device: torch.device,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        if active_mask is None:
            active_mask = torch.ones((num_rows,), dtype=torch.bool, device=device)
        else:
            active_mask = active_mask.to(device=device, dtype=torch.bool).view(-1)
        active_rows = torch.where(active_mask)[0]
        return active_mask, active_rows

    @staticmethod
    def _sample_independent_actions(
        *,
        logits: torch.Tensor,
        active_rows: torch.Tensor,
    ) -> torch.Tensor:
        dist = torch.distributions.Categorical(
            logits=logits.index_select(0, active_rows),
            validate_args=False,
        )
        return dist.sample()

    @staticmethod
    def _compute_group_logits(
        *,
        logits: torch.Tensor,
        group_ids: torch.Tensor,
        num_groups: int,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        num_actions = int(logits.size(1))
        group_sum = torch.zeros(
            (num_groups, num_actions), device=logits.device, dtype=logits.dtype
        )
        group_counts = torch.zeros(
            (num_groups,), device=logits.device, dtype=torch.long
        )
        group_index = group_ids.unsqueeze(1).expand(-1, num_actions)
        group_sum.scatter_add_(0, group_index, logits)
        group_counts.scatter_add_(0, group_ids, torch.ones_like(group_ids))
        group_counts = group_counts.clamp(min=1)
        group_logits = group_sum / group_counts.to(dtype=logits.dtype).unsqueeze(1)
        return group_logits, group_counts

    @staticmethod
    def _compute_within_group_positions(
        *,
        group_ids: torch.Tensor,
        num_groups: int,
    ) -> torch.Tensor:
        if int(group_ids.numel()) == 0:
            return group_ids
        sorted_idx = torch.argsort(group_ids, stable=True)
        sorted_group_ids = group_ids.index_select(0, sorted_idx)
        positions = torch.arange(
            int(group_ids.numel()), device=group_ids.device, dtype=torch.long
        )
        group_start = torch.full(
            (num_groups,),
            fill_value=int(group_ids.numel()),
            device=group_ids.device,
            dtype=torch.long,
        )
        group_start.scatter_reduce_(
            0, sorted_group_ids, positions, reduce="amin", include_self=True
        )
        within_sorted = positions - group_start.index_select(0, sorted_group_ids)
        within_group = torch.empty_like(within_sorted)
        within_group.index_copy_(0, sorted_idx, within_sorted)
        return within_group

    @staticmethod
    def _sample_gumbel_topk(
        *,
        logits: torch.Tensor,
        sample_k: int,
    ) -> torch.Tensor:
        if sample_k <= 0:
            return logits.new_zeros((int(logits.size(0)), 0), dtype=torch.long)
        eps = torch.finfo(logits.dtype).tiny
        uniform = torch.rand_like(logits).clamp(min=eps, max=1.0 - eps)
        gumbel = -torch.log(-torch.log(uniform))
        return torch.topk(logits + gumbel, k=sample_k, dim=1).indices

    def _sample_grouped_actions(
        self,
        *,
        logits: torch.Tensor,
        active_rows: torch.Tensor,
        agent_graph_ids: torch.Tensor,
        source_nodes: torch.Tensor,
        num_nodes_total: int,
        stop_index: int,
    ) -> torch.Tensor:
        inverse, num_groups = self._build_group_index(
            agent_graph_ids=agent_graph_ids,
            source_nodes=source_nodes,
            active_rows=active_rows,
            num_nodes_total=num_nodes_total,
            device=logits.device,
        )
        row_logits = logits.index_select(0, active_rows)
        group_logits, group_counts = self._compute_group_logits(
            logits=row_logits, group_ids=inverse, num_groups=num_groups
        )
        masked_logits, sample_counts, valid_counts = self._prepare_group_sampling(
            group_logits=group_logits, group_counts=group_counts, device=logits.device
        )
        topk_actions, fallback_actions, max_sample = self._draw_group_samples(
            masked_logits=masked_logits,
            sample_counts=sample_counts,
        )
        within_group = self._compute_within_group_positions(
            group_ids=inverse, num_groups=num_groups
        )
        grouped_actions = self._assign_group_actions(
            group_ids=inverse,
            within_group=within_group,
            sample_counts=sample_counts,
            valid_counts=valid_counts,
            topk_actions=topk_actions,
            fallback_actions=fallback_actions,
            max_sample=max_sample,
            stop_index=stop_index,
        )
        return grouped_actions

    @staticmethod
    def _build_group_index(
        *,
        agent_graph_ids: torch.Tensor,
        source_nodes: torch.Tensor,
        active_rows: torch.Tensor,
        num_nodes_total: int,
        device: torch.device,
    ) -> tuple[torch.Tensor, int]:
        keys = agent_graph_ids.to(device=device, dtype=torch.long)
        key_base = max(int(num_nodes_total), 1)
        keys = keys * key_base + source_nodes.to(device=device, dtype=torch.long).clamp(
            min=0
        )
        active_keys = keys.index_select(0, active_rows)
        unique_keys, inverse = torch.unique(
            active_keys, sorted=False, return_inverse=True
        )
        return inverse, int(unique_keys.numel())

    @staticmethod
    def _prepare_group_sampling(
        *,
        group_logits: torch.Tensor,
        group_counts: torch.Tensor,
        device: torch.device,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        valid_mask = torch.isfinite(group_logits)
        valid_counts = valid_mask.sum(dim=1).to(dtype=torch.long)
        sample_counts = torch.minimum(group_counts, valid_counts)
        neg_inf = torch.tensor(float("-inf"), device=device, dtype=group_logits.dtype)
        masked_logits = group_logits.masked_fill(~valid_mask, neg_inf)
        return masked_logits, sample_counts, valid_counts

    def _draw_group_samples(
        self,
        *,
        masked_logits: torch.Tensor,
        sample_counts: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, int]:
        num_groups = int(masked_logits.size(0))
        max_sample = int(sample_counts.max().item()) if num_groups > 0 else 0
        topk_actions = self._sample_gumbel_topk(
            logits=masked_logits,
            sample_k=min(max_sample, int(masked_logits.size(1))),
        )
        fallback_actions = masked_logits.argmax(dim=1)
        return topk_actions, fallback_actions, max_sample

    @staticmethod
    def _assign_group_actions(
        *,
        group_ids: torch.Tensor,
        within_group: torch.Tensor,
        sample_counts: torch.Tensor,
        valid_counts: torch.Tensor,
        topk_actions: torch.Tensor,
        fallback_actions: torch.Tensor,
        max_sample: int,
        stop_index: int,
    ) -> torch.Tensor:
        group_for_rows = group_ids
        row_sample_counts = sample_counts.index_select(0, group_for_rows)
        use_sample = within_group < row_sample_counts
        grouped_actions = torch.full(
            (int(group_ids.numel()),),
            fill_value=stop_index,
            device=group_ids.device,
            dtype=torch.long,
        )
        if max_sample > 0:
            safe_within = within_group.clamp(max=max_sample - 1)
            sampled_actions = topk_actions[group_for_rows, safe_within]
            grouped_actions = torch.where(use_sample, sampled_actions, grouped_actions)
        use_fallback = (~use_sample) & (
            valid_counts.index_select(0, group_for_rows) > 0
        )
        fallback = fallback_actions.index_select(0, group_for_rows)
        grouped_actions = torch.where(use_fallback, fallback, grouped_actions)
        return grouped_actions


class RolloutSampler:
    """Facade over three rollout engines: online, offline-forced, and beam decode."""

    def __init__(self, config: RolloutConfig) -> None:
        self.config = config
        self.action_sampler = ActionSampler()
        backward_prior_mode = str(config.backward_prior_mode).strip().lower()
        if backward_prior_mode != "uniform_in_degree":
            raise ValueError(
                "sampling.backward_prior_mode must be 'uniform_in_degree', "
                f"got {config.backward_prior_mode!r}."
            )
        self.online_engine = OnlineRolloutEngine(
            config=config,
            action_sampler=self.action_sampler,
        )
        self.offline_engine = OfflineForcedEvalEngine(
            config=config,
        )
        self.beam_engine = BeamDecoderEngine(config=config)

    def sample_forward(
        self,
        env_context: GraphEnvContext,
        policy: DualFlowPolicy,
        *,
        flow_direction: FlowDirection = "forward",
        deterministic: bool = False,
        temperature: float | None = None,
        encoded_context: EncodedPolicyContext | None = None,
        collect_traces: bool = True,
    ) -> RolloutResult:
        return self.online_engine.sample_forward(
            env_context,
            policy,
            flow_direction=flow_direction,
            deterministic=deterministic,
            temperature=temperature,
            encoded_context=encoded_context,
            collect_traces=collect_traces,
        )

    def evaluate_forced_paths(
        self,
        env_context: GraphEnvContext,
        policy: DualFlowPolicy,
        *,
        start_local_indices: torch.Tensor,
        forced_edge_ids: torch.Tensor,
        path_lengths: torch.Tensor,
        collect_traces: bool = True,
        use_visited_mask: bool = False,
        encoded_context: EncodedPolicyContext | None = None,
    ) -> RolloutResult:
        return self.offline_engine.evaluate_forced_paths(
            env_context,
            policy,
            start_local_indices=start_local_indices,
            forced_edge_ids=forced_edge_ids,
            path_lengths=path_lengths,
            collect_traces=collect_traces,
            use_visited_mask=use_visited_mask,
            encoded_context=encoded_context,
        )

    def beam_search_forward(
        self,
        env_context: GraphEnvContext,
        policy: DualFlowPolicy,
        *,
        flow_direction: FlowDirection = "forward",
        beam_size: int,
        max_steps: int,
        require_done: bool,
        diverse_penalty: float = 0.0,
        candidate_expansion_factor: int = 8,
        encoded_context: EncodedPolicyContext | None = None,
    ) -> RolloutResult:
        return self.beam_engine.beam_search_forward(
            env_context,
            policy,
            flow_direction=flow_direction,
            beam_size=beam_size,
            max_steps=max_steps,
            require_done=require_done,
            diverse_penalty=diverse_penalty,
            candidate_expansion_factor=candidate_expansion_factor,
            encoded_context=encoded_context,
        )


__all__ = ["ActionSampler", "RolloutResult", "RolloutSampler"]
