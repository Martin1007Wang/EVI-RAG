...
def compute_edge_logits(
    *,
    ...
    edge_question_context = lexical_question_tokens.index_select(
        0, edge_agent_batch
    ).to(dtype=torch.float32)
    edge_question_padding_mask = agent_question_padding_mask.index_select(
        0, edge_agent_batch
    ).to(dtype=torch.bool)
    rel_query = relation_lexical_proj(edge_rel_features)
    rel_query = F.normalize(rel_query, dim=-1, eps=1.0e-6)
    token_repr = F.normalize(edge_question_context, dim=-1, eps=1.0e-6)
    token_similarity = torch.einsum("ed,eld->el", rel_query, token_repr)
    token_similarity = token_similarity.masked_fill(
        edge_question_padding_mask,
        torch.tensor(
            float("-inf"), device=token_similarity.device, dtype=token_similarity.dtype
        ),
    )
    lexical_bias = token_similarity.amax(dim=-1)
    lexical_bias = torch.where(
    ...
    if node_log_h.dim() != 1 or int(node_log_h.numel()) != int(
        ...
        raise ValueError(
            "node_log_h must be 1D with num_nodes_total entries in Doob-h move shift: "
            f"node_log_h={tuple(node_log_h.shape)}, num_nodes_total={int(env_context.num_nodes_total)}."
        )
    safe_target_nodes = target_nodes.clamp(
        min=0, max=max(int(env_context.num_nodes_total) - 1, 0)
    ).to(dtype=torch.long)
    edge_target_log_h = node_log_h.index_select(0, safe_target_nodes).to(
        dtype=torch.float32
    )
    edge_logits = edge_logits + doob_h_alpha * edge_target_log_h
    ...
    # At virtual super-source steps relation selection is degenerate; keep entity-conditional logits only.
    super_edge_logits = conditional_log_prob + doob_h_alpha * edge_target_log_h
