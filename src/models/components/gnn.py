...
class RelationalGNNLayer(nn.Module):
    ...
    def __init__(self, *, hidden_dim: int, dropout: float, avg_d: float = 3.0) -> None:
        ...
        init_linear_xavier(self.msg_proj)
        init_linear_xavier(self.agg_proj)
        init_linear_xavier(self.update_proj)
    ...
    def _safe_pna_stats(
        ...
        *,
        ...
        max_vals = torch.full(
            (num_nodes, hidden_dim), finfo.min, device=device, dtype=dtype
        )
        max_vals.scatter_reduce_(
            0, tail_index, messages, reduce="amax", include_self=False
        )
        min_vals = torch.full(
            (num_nodes, hidden_dim), finfo.max, device=device, dtype=dtype
        )
        min_vals.scatter_reduce_(
            0, tail_index, messages, reduce="amin", include_self=False
        )
    ...
    def _pna_aggregate(
        ...
        *,
        ...
        # stats: [N, D*4]
        stats, deg, has_in = self._safe_pna_stats(
