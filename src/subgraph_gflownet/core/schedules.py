...
def _resolve_progress(
    *, global_step: int, total_steps: int, hold_steps: int = 0
) -> float:
    if total_steps <= 1:
        return 1.0
...
def _resolve_scalar_schedule_value(
    *,
    schedule_type: str,
    initial_value: float,
    ...
    horizon_error: str,
    final_value_error: str,
) -> float:
    if schedule_type == "constant":
        return initial_value

    resolved_total_steps = schedule_context.resolve_horizon(
        explicit_horizon=total_steps,
        interval="step",
    )
    if resolved_total_steps is None:
        raise RuntimeError(horizon_error)
    if final_value is None:
        raise RuntimeError(final_value_error)
...
class SamplingTemperatureScheduler:
    def __init__(
        ...
        *,
        ...
        if self.schedule_type != "constant" and final_temperature is None:
            raise ValueError(
                "training.sampling_temperature_schedule.final_temperature must be set for annealed schedules."
            )
    ...
    def value(
        ...
        *,
        ...
        return _resolve_scalar_schedule_value(
            schedule_type=self.schedule_type,
            initial_value=initial_temperature,
            final_value=self.final_temperature,
            ...
            final_value_error=(
                "sampling temperature schedule requires final_temperature for annealed schedules."
            ),
        )
...
class ReplayMixScheduler:
    def __init__(
        ...
        *,
        ...
        if self.schedule_type != "constant" and final_alpha is None:
            raise ValueError(
