...
def _slice_answer_indices(batch: Any, sample_idx: int) -> torch.Tensor:
    ...
    if isinstance(attr, (list, tuple)):
        ...
        return torch.as_tensor(data, dtype=torch.long).view(-1)
    if torch.is_tensor(attr):
        ...
        if ptr.numel() <= sample_idx + 1:
            return torch.empty(0, dtype=torch.long)
        start = int(ptr[sample_idx].detach().tolist())
        end = int(ptr[sample_idx + 1].detach().tolist())
        return attr[start:end].to(dtype=torch.long).view(-1)
    return torch.empty(0, dtype=torch.long)
...
def _map_answer_indices(
    ...
    node_end = int(node_ptr[sample_idx + 1].detach().tolist())
    if node_end <= node_start:
        return torch.empty(0, dtype=torch.long, device=node_ids.device)
    if node_end > int(node_ids.numel()):
        raise ValueError(
            "node_ptr exceeds node_ids length in extract_answer_entity_ids."
        )
    ...
    in_local = (indices >= 0) & (indices < num_nodes)
...
def compute_answer_recall(
    samples: Iterable[Dict[str, torch.Tensor]], k_values: Sequence[int]
) -> Dict[str, float]:
    ...
    return {
        f"answer_recall@{k}": float(sum(values) / len(values)) if values else 0.0
...
def compute_answer_hit(
    ...
    for sample in samples:
        ...
        prefix_any = edge_hit.cumsum(0) > 0
        for k in ks:
            k_eff = min(k, max_k_eff)
            if k_eff == 0:
                hits[k].append(0.0)
                continue
            found_any = bool(prefix_any[k_eff - 1].detach().tolist())
            hits[k].append(1.0 if found_any else 0.0)
    return {
        f"answer_hit@{k}": float(sum(values) / len(values)) if values else 0.0
...
def _prepare_topk_edges(
    ...
    if max_k_eff == 0:
        empty = order.new_empty((0,), dtype=torch.long)
