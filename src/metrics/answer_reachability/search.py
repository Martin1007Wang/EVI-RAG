...
class _ReachabilityAnalyzerProtocol(Protocol):
    def analyze(
        self,
        *,
...
class ReachabilityGuidedSearch:
    ...
    def generate_window(
        ...
        *,
        ...
        if batch.num_graphs != 1:
            raise ValueError("ReachabilityGuidedSearch expects a single-graph batch.")
        analysis = (
        ...
        while not self._targets_met(covered_mass=covered_mass, targets=targets):
            if not frontier:
                return self._handle_incomplete_search(
                    ...
                    error_message=(
                        "Reachability-guided trajectory search exhausted candidates before covering the selected answer support mass."
                    ),
            ...
            if expansions >= int(self.inference_cfg.max_expansions):
                return self._handle_incomplete_search(
                    ...
                    error_message=(
                        "Reachability-guided trajectory search exceeded max_expansions before covering the selected answer support mass."
                    ),
                    include_answer_support=include_answer_support,
                )
            expansions += 1
            ...
            if len(frontier) > int(self.inference_cfg.max_frontier_size):
                return self._handle_incomplete_search(
                    ...
                    error_message=(
                        "Reachability-guided trajectory search exceeded max_frontier_size before covering the selected answer support mass."
                    ),
                    include_answer_support=include_answer_support,
                )
    ...
    def _handle_incomplete_search(
        ...
        *,
        ...
        discovered_paths: list[DiscoveredTrajectory],
        frontier: list[tuple[float, int, _PrefixCandidate]],
        expansions: int,
        stop_reason: str,
        error_message: str,
        include_answer_support: bool = True,
    ) -> SupportWindowResult:
        if bool(self.inference_cfg.strict_search):
    ...
    def _initialize_frontier(
        ...
        *,
        ...
        prepared_batch: PreparedSearchBatch,
        analysis: ExactReachabilityAnalysis,
    ) -> tuple[list[tuple[float, int, _PrefixCandidate]], int]:
        frontier: list[tuple[float, int, _PrefixCandidate]] = []
    ...
    @staticmethod
    def _remaining_mass(frontier: list[tuple[float, int, _PrefixCandidate]]) -> float:
        return float(
            sum(
