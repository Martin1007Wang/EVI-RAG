...
from src.weaver.losses import SubTrajectoryBalanceLoss, stop_now_tb_loss
    ...
    *,
    ...
    return RolloutBatch(
        stats=RolloutStats(
            ...
            proposal_intervention_count=zeros_b.clone(),
            edge_action_entropy=zeros_b.clone(),
            edge_action_entropy_valid_mask=zeros_b.clone(),
        ),
...
def test_subtrajectory_balance_default_stop_tb_coef_preserves_total_loss() -> None:
    rollout = _rollout_for_loss(
        state_log_flows=torch.tensor([[0.0]], dtype=torch.float32),
        step_log_pf=torch.tensor([[0.0]], dtype=torch.float32),
        terminal_log_reward=torch.tensor([0.0], dtype=torch.float32),
    ...
    assert output.metrics["loss/total"].item() == pytest.approx(
        output.metrics["loss/subtb"].item()
    )


def test_subtrajectory_balance_adds_annealed_advantage_aux_loss() -> None:
    rollout = _rollout_for_loss(
        state_log_flows=torch.tensor([[0.0]], dtype=torch.float32),
        step_log_pf=torch.tensor([[0.0]], dtype=torch.float32),
        terminal_log_reward=torch.tensor([0.0], dtype=torch.float32),
        stop_log_pf=torch.tensor([[0.0]], dtype=torch.float32),
        stop_now_log_reward=torch.tensor([[0.0]], dtype=torch.float32),
        stop_tb_valid_mask=torch.tensor([[False]], dtype=torch.bool),
    )
    rollout.traces.advantage_aux_loss[0, 0] = torch.tensor(0.7)
    rollout.traces.advantage_aux_valid_mask[0, 0] = True

    loss_fn = SubTrajectoryBalanceLoss(
        max_trajectory_len=1,
        advantage_aux_coef_start=1.0,
        advantage_aux_coef_end=0.5,
        advantage_aux_anneal_steps=10,
    )
    loss_fn.set_global_step(5)

    output = loss_fn(rollout)

    assert output.metrics["loss/advantage_aux"].item() == pytest.approx(0.7)
    assert output.metrics["loss/advantage_aux_coef"].item() == pytest.approx(0.75)
    assert output.metrics["loss/total"].item() == pytest.approx(0.525)
