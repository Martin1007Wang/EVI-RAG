...
from src.data.llm_reasoner_path_dataset import LLMReasonerPathDataset


def test_llm_reasoner_path_dataset_uses_traversal_direction() -> None:
    # Bypass __init__ (it requires filesystem parquet caches); we only validate formatting logic.
    dataset = LLMReasonerPathDataset.__new__(LLMReasonerPathDataset)
