...
def test_root_answer_edge_rank_excludes_missing_graphs() -> None:
    ...
    assert metrics["prior/root_answer_edge_best_rank_mean"] == pytest.approx(1.0)
    assert metrics["prior/root_answer_edge_best_rank_median"] == pytest.approx(1.0)
    assert metrics["prior/root_answer_edge_top1_rate"] == pytest.approx(1.0)
    assert metrics["prior/root_answer_edge_top5_rate"] == pytest.approx(1.0)
    assert metrics["prior/root_answer_edge_mrr"] == pytest.approx(1.0)
    assert metrics["target_policy/root_answer_edge_best_rank_mean"] == pytest.approx(1.0)
    assert metrics["target_policy/root_answer_edge_best_rank_median"] == pytest.approx(1.0)
    assert metrics["target_policy/root_answer_edge_top1_rate"] == pytest.approx(1.0)
    assert metrics["target_policy/root_answer_edge_top5_rate"] == pytest.approx(1.0)
    assert metrics["target_policy/root_answer_edge_mrr"] == pytest.approx(1.0)
    assert metrics["target_policy/root_answer_edge_rank_delta_mean"] == pytest.approx(0.0)
    assert metrics["target_policy/root_worse_than_prior_rate"] == pytest.approx(0.0)
    assert metrics["prior/root_logit_std"] == pytest.approx(
        float(torch.tensor([1.0, 2.0, 3.0]).std(unbiased=False).item())
    )
    assert metrics["prior/target_policy_rank_kendall"] == pytest.approx(1.0)
    assert metrics["prior/answer_edge_rank"] == pytest.approx(1.0)
    assert metrics["target_policy/answer_edge_rank"] == pytest.approx(1.0)

    assert metrics["prior/root_answer_edge_q_rel_mean"] == pytest.approx(0.9)
    assert metrics["prior/root_answer_edge_q_new_mean"] == pytest.approx(0.8)
    assert metrics["prior/root_answer_edge_q_candidate_mean"] == pytest.approx(0.7)
    assert metrics["prior/root_answer_edge_new_text_rate"] == pytest.approx(1.0)
    assert metrics["prior/root_answer_edge_logit_mean"] == pytest.approx(2.0)
    assert metrics["target_policy/root_answer_edge_logit_mean"] == pytest.approx(2.0)
    assert metrics["prior/root_nonanswer_edge_q_rel_mean"] == pytest.approx(0.2)
    assert metrics["prior/root_nonanswer_edge_q_new_mean"] == pytest.approx(0.3)
    assert metrics["prior/root_nonanswer_edge_q_candidate_mean"] == pytest.approx(0.4)
    assert metrics["prior/root_nonanswer_edge_new_text_rate"] == pytest.approx(0.5)
    assert metrics["prior/root_nonanswer_edge_logit_mean"] == pytest.approx(2.0)
    assert metrics["target_policy/root_nonanswer_edge_logit_mean"] == pytest.approx(2.0)
...
def test_training_policy_diagnostics_emit_root_edge_prior_metrics() -> None:
    ...
    assert metrics["train/prior/root_logit_std"] > 0.0
    assert metrics["train/prior/answer_edge_rank"] == pytest.approx(1.0)
    assert metrics["train/target_policy/answer_edge_rank"] == pytest.approx(1.0)
