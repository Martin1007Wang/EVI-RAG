...
def test_reward_engine_distance_shaping_and_unreachable_fallback() -> None:
    context = _make_context()
    engine = DualFlowRewardEngine(
        stop_cfg=StopConfig(
            reward_base=1.0,
            reward_epsilon=1.0e-6,
            distance_reward_gamma=0.5,
    ...
    assert float(rewards_forward[0, 1].item()) == pytest.approx(0.5, rel=1e-6, abs=1e-6)
    assert float(rewards_forward[0, 2].item()) == pytest.approx(0.25, rel=1e-6, abs=1e-6)
    ...
    assert float(rewards_backward[0, 2].item()) == pytest.approx(1.0, rel=1e-6, abs=1e-6)
    assert float(rewards_backward[0, 0].item()) == pytest.approx(1.0e-6, rel=1e-6, abs=1e-6)
    assert float(rewards_backward[0, 1].item()) == pytest.approx(1.0e-6, rel=1e-6, abs=1e-6)
