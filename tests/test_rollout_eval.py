...
def _rollout(
    *,
    ...
    horizon: int = 3,
    terminal_edge_penalty: float | None = None,
    terminal_base_log_reward: float | None = None,
    terminal_utility: float | None = None,
    terminal_expanded_edge_count: float | None = None,
    terminal_minimal_edge_count: float | None = None,
    terminal_minimality_gap: float | None = None,
    terminal_minimality_penalty: float | None = None,
    ...
    return RolloutBatch(
        stats=RolloutStats(
            ...
            proposal_intervention_count=torch.zeros(1, dtype=torch.float32),
            edge_action_entropy=torch.tensor([edge_entropy_sum], dtype=torch.float32),
            edge_action_entropy_valid_mask=torch.tensor(
                [edge_entropy_count], dtype=torch.float32
            ),
            terminal_edge_penalty=(
                None
                if terminal_edge_penalty is None
                else torch.tensor([terminal_edge_penalty], dtype=torch.float32)
            ),
            ...
            terminal_minimal_edge_count=(
                None
                if terminal_minimal_edge_count is None
                else torch.tensor([terminal_minimal_edge_count], dtype=torch.float32)
            ),
            terminal_minimality_gap=(
                None
                if terminal_minimality_gap is None
                else torch.tensor([terminal_minimality_gap], dtype=torch.float32)
            ),
            terminal_minimality_penalty=(
                None
                if terminal_minimality_penalty is None
                else torch.tensor([terminal_minimality_penalty], dtype=torch.float32)
            ),
        ),
        traces=RolloutTraces(
            ...
            advantage_aux_loss=zeros_bt.clone(),
            advantage_aux_valid_mask=bool_bt.clone(),
            proposal_intervention_mask=bool_bt.clone(),
    ...
    *,
    ...
    return RolloutBatch(
        stats=RolloutStats(
            ...
            proposal_intervention_count=torch.zeros(batch_size, dtype=torch.float32),
            edge_action_entropy=torch.zeros(batch_size, dtype=torch.float32),
            edge_action_entropy_valid_mask=torch.zeros(batch_size, dtype=torch.float32),
        ),
        traces=RolloutTraces(
            state_log_flows=zeros_bt.clone(),
            log_pf=zeros_bt.clone(),
...
def test_training_diagnostics_collects_reward_breakdown_metrics() -> None:
    ...
    rollout_a = _rollout(
        ...
        terminal_edge_penalty=0.1,
        terminal_base_log_reward=-9.6,
        terminal_utility=0.2,
        terminal_expanded_edge_count=3.0,
        terminal_minimal_edge_count=2.0,
        terminal_minimality_gap=1.0,
        terminal_minimality_penalty=0.15,
    )
    rollout_b = _rollout(
        expand_edges=[0],
        terminal_answer_f1=1.0,
        edge_entropy_sum=0.0,
        edge_entropy_count=0.0,
        terminal_edge_penalty=0.3,
        terminal_base_log_reward=0.0,
        terminal_utility=1.0,
        terminal_expanded_edge_count=1.0,
        terminal_minimal_edge_count=1.0,
        terminal_minimality_gap=0.0,
        terminal_minimality_penalty=0.0,
    )
    collector = TrainingDiagnosticsCollector(debug=False)
    ...
    assert metrics["train/reward/edge_penalty_mean"] == pytest.approx(0.2)
    ...
    assert metrics["train/reward/minimal_edge_count_mean"] == pytest.approx(1.5)
    assert metrics["train/reward/minimality_gap_mean"] == pytest.approx(0.5)
    assert metrics["train/reward/minimality_penalty_mean"] == pytest.approx(0.075)
