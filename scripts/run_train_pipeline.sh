...
# Train pipeline: retriever -> g_agent cache -> gflownet.
#
# Usage:
#   bash scripts/run_train_pipeline.sh <dataset>
...
DATASET="${1:-}"
if [[ -z "${DATASET}" ]]; then
  echo "Usage: bash scripts/run_train_pipeline.sh <dataset>" >&2
...
echo "==> [1/3] train_retriever (experiment=train_retriever_default)"
...
echo "==> [2/3] cache_g_agent (train/val/test)"
...
echo "==> [3/3] train_gflownet (experiment=train_gflownet_default)"
